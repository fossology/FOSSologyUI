...
const MyRecentJobsPage = () => {
  ...
  useEffect(() => {
    ...
    getAllJob(jobsData)
      .then((res) => {
        const formattedJobs = res?.res?.map((job) => ({
          ...job,
          queueDate: formatDate(job.queueDate),
        })) || [];

        setJobsDataList(formattedJobs);
        setPages(res.pages || 1);

        const options = Array.from({ length: res.pages || 1 }, (_, i) => ({
          id: i + 1,
          value: i + 1,
        }));
...
export default MyRecentJobsPage;
