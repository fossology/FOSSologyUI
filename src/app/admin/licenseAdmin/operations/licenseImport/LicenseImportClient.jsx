  ...
  try {
    ...
    for (const file of uploadableFiles) {
      ...
      if (ext === "csv") {
        ...
        response = await importLicenseCsv(formData);
      } else {
        formData.append("fileInput", file);

        response = await importLicenseJson(formData);
      }
...
const handleBack = () => {
    router.push(routes.admin.license.operations);
};
...
return (
    ...
    <p className="mb-6 max-w-5xl text-base text-gray-900">
        This option permits uploading a CSV or JSON file from your computer
        to FOSSology.
        <br />
    ...
    <form onSubmit={handleSubmit}>
        ...
        <AlertBanner
            type="Warning"
            description="NOTE: Obligations will not get imported, To import go to Admin -> Obligation Admin -> Obligation Import"
            showClose={false}
            className="mt-4"
        />
